  ...
  methods: {
    expand(expanded, record) {
	  if(expanded){
		...
		this.expandedRowKeys.splice(this.expandedRowKeys.indexOf(record.key),1)
	  }
	  
    },
    //递归遍历表格数据增加actionList项
    handlerTable(rawData) {
      rawData.forEach(permission => {
        if (permission.children && permission.children.length) {
          this.handlerTable(permission.children)
        }
        if (permission.actionData) {
